    ...
    fn from(target: &scap::Target) -> Self {
        match target {
            scap::Target::Display(d) => {
                TargetInfo { id: d.id, kind: TargetKind::Display, title: d.title.clone() }
            }
            scap::Target::Window(w) => {
                TargetInfo { id: w.id, kind: TargetKind::Window, title: w.title.clone() }
            }
        }
    }
}
...
pub fn list_targets() -> Vec<TargetInfo> {
    ...
    let mut displays: Vec<TargetInfo> = targets
        ...
        .map(TargetInfo::from)
        .collect();
    let windows = targets.iter().filter(|t| matches!(t, scap::Target::Window(_))).map(TargetInfo::from);
...
pub fn resolve_target(id: u32, kind: TargetKind) -> Option<scap::Target> {
    scap::get_all_targets().into_iter().find(|t| match (t, kind) {
